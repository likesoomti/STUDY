...
2. 일반화 관계
   ...
   한 클래스가, 다른 클래스를 포함하는 상위 개념일 때, 일반화 관계

3. 집합 관계

   클래스 사이의 전체-부분 관계를 나타냅니다. 집약 관계와 합성 관계가 존재합니다.

4. 의존 관계

   클래스가 다른 클래스에서 제공하는 기능을 사용할 때 나타냅니다. 연관과의 차이는 한 메서드를 실행하는 동안 유지합니다.

5. 실체화 관계

   인터페이스와 이 책임을 실제로 실현한 클래스들 사이의 관계를 나타냅니다.
